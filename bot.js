...
client.on('guildMemberAdd' , member => {
	...
	if(member.bot == true)
	{
		...
		if (member.guild.greetingChannel == null){
			...
			{
				member.guild.channels.array()[0].send("**Welcome !** <@" + member.id + "> to the server : " + member.guild.name + " !!!\n*If this message is sent in the wrong channel , please set the greeting channel using " + member.guild.commandPrefix + "setgreetingchannel*")
			} else {
				member.guild.channels.array()[0].send(member.guild.welcomeMessage.replace("%user%","<@" + member.id + ">") + "\n*If this message is sent in the wrong channel , please set the greeting channel using " + member.guild.commandPrefix + "setgreetingchannel*")
			}
		} else {
			if(member.guild.welcomeMessage == null)
			{
...
client.on('guildMemberRemove' , member => {
	...
	if(member.bot == true)
	{
		...
		member.guild.channels.array()[0].send("**The bot : <@" + member.id + "> was removed from : " + member.guild.name + " !**\n*If this message is sent in the wrong channel , please set the greeting channel using " + member.guild.commandPrefix + "setgreetingchannel*")
		}
	}
	else{
		if (member.guild.greetingChannel == null){
			if(member.guild.welcomeMessage == null)
			{
				member.guild.channels.array()[0].send("**Goodbye** <@" + member.id + "> , we're gonna miss you in : " + member.guild.name + " !!!\n*If this message is sent in the wrong channel , please set the greeting channel using " + member.guild.commandPrefix + "setgreetingchannel*")
			} else {
				member.guild.channels.array()[0].send(member.guild.welcomeMessage.replace("%user%","<@" + member.id + ">") + "\n*If this message is sent in the wrong channel , please set the greeting channel using " + member.guild.commandPrefix + "setgreetingchannel*")
			}
		} else {
